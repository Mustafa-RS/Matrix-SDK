...
class Qlcrc(object):
    ...
    '''
        Bits should be the crc no. of bits (default to 32).
        The implementation can be extended to 
        crc computations for 8, 16, etc.
        Create the mask value from no. of bits.
        for crc32: 
            mask = 0xffffffff
            shift_bits = 24
    '''
    def __init__(self, bits=32, crc=0):
        """ constructor """
        self.bits = bits
        self.crc = crc
        if bits == 32:
            ...
            self.table = self.crc32_table

    def init_crc(self, val):
        self.crc = val
    ...
    def calc_crc(self, data):
        ...
        """
        if self.bits != 32:
            logging.error("error: CRC-%d computation is not supported." 
                          " Use only 32(default) bits",
                          self.bits)
            return Status.FAIL

        self.init_crc(self.mask)
        for each_byte in bytearray(data):
            self.crc = ((self.crc << 8) ^
                        self.table[((self.crc >> self.shift_bits) ^
                                    each_byte) & 0xff]) & self.mask

        return 


def get_crc32_from_string(buffer, crc=0):
    """ Compute CRC32 for the input buffer data.
    Use zero initial value for crc.
    """
    crc_obj = Qlcrc(crc=crc)
    crc_obj.calc_crc(buffer)
    return crc_obj.crc


def get_crc32(in_file, crc=0):
    """Compute 32-bit CRC for the input file data.
    Use zero as the default initial value for crc.
    """
    crc_obj = Qlcrc(crc=crc)
    with open (in_file, 'rb') as file_data:
        crc_obj.calc_crc(file_data.read())
    return crc_obj.crc
